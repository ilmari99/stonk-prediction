...
def plot_mask_and_data(mask, price_data, ind_conversion:dict=None):
    ...
    """
    assert mask.shape == price_data.shape, \
    ...
    for stock_idx in range(mask.shape[1]):
        ...
        ax[stock_idx].scatter(buy_idx,
                              price_data[buy_idx,stock_idx],
                              color="green", label="Buy")
        ax[stock_idx].scatter(sell_idx,
                              price_data[sell_idx,stock_idx],
...
def plot_strategy_based_on_predictions(data, transformed_data, model,
    ...
    invest_strat = \
        strategy_mask_from_price_model(data,
                                       transformed_data,
                                       window_hours,
                                       model,
                                       inversion=inversion) \
        if model_type == "price"\
        else strategy_mask_from_updown_model(transformed_data,
                                             window_hours,
                                             model,
                                             output_scale=output_scale)

    print(f"""
            Profit on invest strategy:
    ...
    for i in range(3):
        for j in range(2):
            ...
            ax[i,j].scatter(sell_idx, data[sell_idx,stock_idx],
                            color="red", label="Sell")
            ax[i,j].scatter(buy_idx, data[buy_idx,stock_idx],
                            color="green", label="Buy")
